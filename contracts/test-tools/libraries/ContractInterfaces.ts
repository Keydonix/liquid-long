...
class CallableContract {
	...
	public constructor(contract: Contract, gasPriceInNanoeth: number) {
		...
		this.gasPrice = utils.bigNumberify(gasPriceInNanoeth * 10 ** 9);
	}

	protected async localCall(parameters: Array<any>, txName: string, sender?: string, attachedEth?: utils.BigNumber): Promise<Array<any>> {
		var overrideOptions: EthersOptions = {};
		if (attachedEth !== undefined) overrideOptions["value"] = utils.bigNumberify(attachedEth);
		return [await this.contract.functions[txName.toString()](...parameters, overrideOptions)];
	}

	protected async remoteCall(parameters: Array<any>, txName: string, sender?: string, gasPrice?: utils.BigNumber, attachedEth?: utils.BigNumber): Promise<string|undefined> {
		var overrideOptions: EthersOptions = {};
		if (this.gasPrice !== undefined) overrideOptions["gasPrice"] = this.gasPrice;
		if (gasPrice !== undefined) overrideOptions["gasPrice"] = utils.bigNumberify(gasPrice);
		if (attachedEth !== undefined) overrideOptions["value"] = utils.bigNumberify(attachedEth);
...
export class OasisdexContract extends CallableContract {
  ...
  public getBestOffer_ = async (sell_gem: string, buy_gem: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([sell_gem, buy_gem], "getBestOffer", options.sender);
    return <utils.BigNumber>result[0];
  }

  public sellAllAmount = async (pay_gem: string, pay_amt: utils.BigNumber, buy_gem: string, min_fill_amount: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([pay_gem, pay_amt, buy_gem, min_fill_amount], "sellAllAmount", options.sender, options.gasPrice);
    return;
  }

  public stop = async (options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([], "stop", options.sender, options.gasPrice);
    return;
  }

  public make = async (pay_gem: string, buy_gem: string, pay_amt: utils.BigNumber, buy_amt: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([pay_gem, buy_gem, pay_amt, buy_amt], "make", options.sender, options.gasPrice);
    return;
  }

  public setOwner = async (owner_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([owner_], "setOwner", options.sender, options.gasPrice);
    return;
  }

  public getBuyAmount_ = async (buy_gem: string, pay_gem: string, pay_amt: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([buy_gem, pay_gem, pay_amt], "getBuyAmount", options.sender);
    return <utils.BigNumber>result[0];
  }

  public addTokenPairWhitelist = async (baseToken: string, quoteToken: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([baseToken, quoteToken], "addTokenPairWhitelist", options.sender, options.gasPrice);
    return;
  }

  public remTokenPairWhitelist = async (baseToken: string, quoteToken: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([baseToken, quoteToken], "remTokenPairWhitelist", options.sender, options.gasPrice);
    return;
  }

  public offer = async (pay_amt: utils.BigNumber, pay_gem: string, buy_amt: utils.BigNumber, buy_gem: string, pos: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([pay_amt, pay_gem, buy_amt, buy_gem, pos], "offer", options.sender, options.gasPrice);
    return;
  }

  public insert = async (id: utils.BigNumber, pos: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([id, pos], "insert", options.sender, options.gasPrice);
    return;
  }

  public last_offer_id_ = async (options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([], "last_offer_id", options.sender);
    return <utils.BigNumber>result[0];
  }

  public setMatchingEnabled = async (matchingEnabled_: boolean, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([matchingEnabled_], "setMatchingEnabled", options.sender, options.gasPrice);
    return;
  }

  public cancel = async (id: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([id], "cancel", options.sender, options.gasPrice);
    return;
  }

  public getOffer_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<Array<string>> => {
    options = options || {};
    const result = await this.localCall([id], "getOffer", options.sender);
    return <Array<string>>result;
  }

  public del_rank = async (id: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([id], "del_rank", options.sender, options.gasPrice);
    return;
  }

  public take = async (id: string, maxTakeAmount: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([id, maxTakeAmount], "take", options.sender, options.gasPrice);
    return;
  }

  public getMinSell_ = async (pay_gem: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([pay_gem], "getMinSell", options.sender);
    return <utils.BigNumber>result[0];
  }

  public getTime_ = async (options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([], "getTime", options.sender);
    return <utils.BigNumber>result[0];
  }

  public getNextUnsortedOffer_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([id], "getNextUnsortedOffer", options.sender);
    return <utils.BigNumber>result[0];
  }

  public close_time_ = async (options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([], "close_time", options.sender);
    return <utils.BigNumber>result[0];
  }

  public _span_ = async (arg0: string, arg1: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([arg0, arg1], "span", options.sender);
    return <utils.BigNumber>result[0];
  }

  public _best_ = async (arg0: string, arg1: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
  ...
  public bump = async (id_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([id_], "bump", options.sender, options.gasPrice);
    return;
  }

  public setAuthority = async (authority_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([authority_], "setAuthority", options.sender, options.gasPrice);
    return;
  }

  public getOfferCount_ = async (sell_gem: string, buy_gem: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([sell_gem, buy_gem], "getOfferCount", options.sender);
    return <utils.BigNumber>result[0];
  }

  public buyAllAmount = async (buy_gem: string, buy_amt: utils.BigNumber, pay_gem: string, max_fill_amount: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([buy_gem, buy_amt, pay_gem, max_fill_amount], "buyAllAmount", options.sender, options.gasPrice);
    return;
  }

  public isActive_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<boolean> => {
    options = options || {};
    const result = await this.localCall([id], "isActive", options.sender);
    return <boolean>result[0];
  }

  public offers_ = async (arg0: utils.BigNumber, options?: { sender?: string }): Promise<Array<string>> => {
    options = options || {};
    const result = await this.localCall([arg0], "offers", options.sender);
    return <Array<string>>result;
  }

  public getFirstUnsortedOffer_ = async (options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
  ...
  public getBetterOffer_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([id], "getBetterOffer", options.sender);
    return <utils.BigNumber>result[0];
  }

  public _dust_ = async (arg0: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([arg0], "_dust", options.sender);
    return <utils.BigNumber>result[0];
  }

  public getWorseOffer_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
  ...
  public _near_ = async (arg0: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
    const result = await this.localCall([arg0], "_near", options.sender);
    return <utils.BigNumber>result[0];
  }

  public kill = async (id: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
  ...
  public setMinSell = async (pay_gem: string, dust: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
  ...
  public _rank_ = async (arg0: utils.BigNumber, options?: { sender?: string }): Promise<Array<string>> => {
    options = options || {};
    const result = await this.localCall([arg0], "_rank", options.sender);
    return <Array<string>>result;
  }

  public getOwner_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<string> => {
    options = options || {};
    const result = await this.localCall([id], "getOwner", options.sender);
    return <string>result[0];
  }

  public isOfferSorted_ = async (id: utils.BigNumber, options?: { sender?: string }): Promise<boolean> => {
    options = options || {};
    const result = await this.localCall([id], "isOfferSorted", options.sender);
    return <boolean>result[0];
  }

  public setBuyEnabled = async (buyEnabled_: boolean, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
    await this.remoteCall([buyEnabled_], "setBuyEnabled", options.sender, options.gasPrice);
    return;
  }

  public buy = async (id: utils.BigNumber, amount: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
    options = options || {};
  ...
  public getPayAmount_ = async (pay_gem: string, buy_gem: string, buy_amt: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
    options = options || {};
...
export class SaiMom extends CallableContract {
	...
	public setTubGap = async(wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([wad], "setTubGap", options.sender, options.gasPrice);
		return;
	}

	public setTubGap_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([wad], "setTubGap", options.sender);

	}

	public setTapGap = async(wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([wad], "setTapGap", options.sender, options.gasPrice);
		return;
	}

	public setTapGap_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([wad], "setTapGap", options.sender);

	}

	public setTax = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setTax", options.sender, options.gasPrice);
		return;
	}

	public setTax_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public setCap = async(wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([wad], "setCap", options.sender, options.gasPrice);
		return;
	}

	public setCap_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public setFee = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setFee", options.sender, options.gasPrice);
		return;
	}

	public setFee_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public setMat = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setMat", options.sender, options.gasPrice);
		return;
	}

	public setMat_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([ray], "setMat", options.sender);

	}

	public setPip = async(pip_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public setHow = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setHow", options.sender, options.gasPrice);
		return;
	}

	public setHow_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([ray], "setHow", options.sender);

	}

	public setAxe = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setAxe", options.sender, options.gasPrice);
		return;
	}

	public setAxe_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([ray], "setAxe", options.sender);

	}

	public setWay = async(ray: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([ray], "setWay", options.sender, options.gasPrice);
		return;
	}

	public setWay_ = async(ray: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public setVox = async(vox_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public setPep = async(pep_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
...
export class SaiTub extends CallableContract {
	...
	public join = async(wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([wad], "join", options.sender, options.gasPrice);
		return;
	}

	public join_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public era_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "era", options.sender);
		return <utils.BigNumber>result[0];
	}

	public ink_ = async(cup: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([cup], "ink", options.sender);
		return <utils.BigNumber>result[0];
	}

	public rho_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "rho", options.sender);
		return <utils.BigNumber>result[0];
	}

	public air_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "air", options.sender);
		return <utils.BigNumber>result[0];
	}

	public rhi = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([], "rhi", options.sender, options.gasPrice);
		return;
	}

	public rhi_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "rhi", options.sender);
		return <utils.BigNumber>result[0];
	}

	public flow = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public cap_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "cap", options.sender);
		return <utils.BigNumber>result[0];
	}

	public bite = async(cup: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public draw = async(cup: string, wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup, wad], "draw", options.sender, options.gasPrice);
		return;
	}

	public draw_ = async(cup: string, wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([cup, wad], "draw", options.sender);

	}

	public bid_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([wad], "bid", options.sender);
		return <utils.BigNumber>result[0];
	}

	public cupi_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "cupi", options.sender);
		return <utils.BigNumber>result[0];
	}

	public axe_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "axe", options.sender);
		return <utils.BigNumber>result[0];
	}

	public tag_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
	...
	public gap_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "gap", options.sender);
		return <utils.BigNumber>result[0];
	}

	public rap = async(cup: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup], "rap", options.sender, options.gasPrice);
		return;
	}

	public rap_ = async(cup: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([cup], "rap", options.sender);
		return <utils.BigNumber>result[0];
	}

	public wipe = async(cup: string, wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup, wad], "wipe", options.sender, options.gasPrice);
		return;
	}

	public wipe_ = async(cup: string, wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public turn = async(tap_: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public per_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "per", options.sender);
		return <utils.BigNumber>result[0];
	}

	public exit = async(wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([wad], "exit", options.sender, options.gasPrice);
		return;
	}

	public exit_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
	...
	public pie_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "pie", options.sender);
		return <utils.BigNumber>result[0];
	}

	public cage = async(fit_: utils.BigNumber, jam: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([fit_, jam], "cage", options.sender, options.gasPrice);
		return;
	}

	public cage_ = async(fit_: utils.BigNumber, jam: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([fit_, jam], "cage", options.sender);

	}

	public rum_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
	...
	public mold = async(param: string, val: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([param, val], "mold", options.sender, options.gasPrice);
		return;
	}

	public mold_ = async(param: string, val: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([param, val], "mold", options.sender);

	}

	public tax_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "tax", options.sender);
		return <utils.BigNumber>result[0];
	}

	public drip = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public free = async(cup: string, wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup, wad], "free", options.sender, options.gasPrice);
		return;
	}

	public free_ = async(cup: string, wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([cup, wad], "free", options.sender);

	}

	public mat_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
	...
	public lock = async(cup: string, wad: utils.BigNumber, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup, wad], "lock", options.sender, options.gasPrice);
		return;
	}

	public lock_ = async(cup: string, wad: utils.BigNumber, options?: { sender?: string }): Promise<void> => {
		options = options || {};
		await this.localCall([cup, wad], "lock", options.sender);

	}

	public shut = async(cup: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public give = async(cup: string, guy: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public fit_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "fit", options.sender);
		return <utils.BigNumber>result[0];
	}

	public chi = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([], "chi", options.sender, options.gasPrice);
		return;
	}

	public chi_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
	...
	public fee_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
	...
	public din = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([], "din", options.sender, options.gasPrice);
		return;
	}

	public din_ = async( options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([], "din", options.sender);
		return <utils.BigNumber>result[0];
	}

	public ask_ = async(wad: utils.BigNumber, options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([wad], "ask", options.sender);
		return <utils.BigNumber>result[0];
	}

	public safe = async(cup: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
	...
	public tab = async(cup: string, options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
		await this.remoteCall([cup], "tab", options.sender, options.gasPrice);
		return;
	}

	public tab_ = async(cup: string, options?: { sender?: string }): Promise<utils.BigNumber> => {
		options = options || {};
		const result = await this.localCall([cup], "tab", options.sender);
		return <utils.BigNumber>result[0];
	}

	public open = async( options?: { sender?: string, gasPrice?: utils.BigNumber }): Promise<void> => {
		options = options || {};
