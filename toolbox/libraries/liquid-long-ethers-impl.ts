import { Dependencies, Transaction, TransactionReceipt, Bytes32, Bytes } from './liquid-long'
...
export class ContractDependenciesEthers implements Dependencies<ethers.utils.BigNumber> {
	public constructor(private readonly provider: Provider, private readonly signer: Signer, private readonly getGasPriceNanoeth: () => Promise<number>) {}
	...
	submitTransaction = async (transaction: Transaction<ethers.utils.BigNumber>): Promise<TransactionReceipt> => {
		...
		ethersTransaction.gasPrice = ethers.utils.bigNumberify(await this.getGasPriceNanoeth() * 1e9)
